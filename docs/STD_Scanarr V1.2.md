# Scanarr — Spécification Technique Détaillée (STD) — V1.2
...
Table de corrélation pour les suppressions async via le watcher. Chaque commande `command.files.delete` envoyée au watcher est trackée ici.

```sql
CREATE TABLE deletion_requests (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    status VARCHAR(30) NOT NULL DEFAULT 'pending',
    -- 'pending' : commande envoyée au watcher, en attente de résultat
    -- 'waiting_watcher' : watcher offline, sera renvoyé à la reconnexion
    -- 'processing' : watcher a commencé à traiter
    -- 'completed' : tous les fichiers traités
    -- 'failed' : erreur fatale
    source VARCHAR(30) NOT NULL,
    -- 'movie_delete', 'file_delete', 'file_global_delete', 'scheduled_deletion'
    source_entity_id UUID,              -- ID de l'entité source (movie, file, scheduled_deletion)
    files JSONB NOT NULL DEFAULT '[]',  -- payload envoyée au watcher [{media_file_id, host_path, volume_host_path}]
    results JSONB NOT NULL DEFAULT '[]', -- résultats reçus du watcher [{media_file_id, status, dirs_removed, error}]
    context JSONB,                       -- contexte pour le post-traitement (deletion_id, flags, etc.)
...
{
  "data": {
    "message": "Deletion initiated, files being deleted by watcher",
    "request_id": "uuid-deletion-request",
    "watcher_available": true,
    "radarr_dereferenced": true,
    "radarr_auto_search_disabled": true,
    "qbittorrent_cleaned": 1
  }
}
// Response 200 (watcher offline)
{
  "data": {
    "message": "Watcher offline, deletion queued for reconnection",
    "request_id": "uuid-deletion-request",
...
**Flux asynchrone** : La suppression physique des fichiers et le nettoyage des dossiers sont délégués au watcher via WebSocket. L'API retourne immédiatement avec un `request_id` qui permet de suivre l'avancement. La suppression des entrées en BDD et le refresh Plex/Jellyfin se font automatiquement quand le watcher renvoie les résultats.
...
3. Cron quotidien (ProcessScheduledDeletionsCommand) vérifie chaque jour à 23h55 :
   ...
   Phase 2 — Watcher (actions filesystem, déléguées via WebSocket) :
     d. Créer une DeletionRequest en BDD avec la liste des fichiers
     e. Envoyer la commande command.files.delete au watcher via WebSocket
     f. Si le watcher est offline :
        → DeletionRequest.status = "waiting_watcher"
   ...
   Phase 3 — Watcher traite (sur le serveur des fichiers) :
     g. Pour chaque fichier : unlink() le fichier physique
     h. Pour chaque fichier supprimé : nettoyer les dossiers parents vides
        (remonter récursivement jusqu'au volume_host_path, ne jamais supprimer le root)
     i. Renvoyer command.files.delete.progress pour chaque fichier (succès ou échec)
     j. Renvoyer command.files.delete.completed quand tous traités

   Phase 4 — API (à réception des résultats du watcher) :
     k. Pour chaque fichier supprimé : supprimer l'entrée media_file en BDD
     l. Refresh bibliothèques Plex/Jellyfin (API HTTP, best-effort)
     m. Générer le execution_report (JSON)
     n. Status → "completed" (ou "failed" si TOUS les items ont échoué)
     o. Envoyer notification Discord de confirmation
...
**Paths** : Le watcher utilise `volume.host_path` (chemin sur le serveur B). L'API utilise `volume.path` (chemin dans son container). La commande envoyée au watcher contient toujours le `host_path`.
...
|-----------|------|-------------|
| API → Watcher | `command.files.delete` | Commande avec request_id + liste de fichiers (host_path, volume_host_path, media_file_id) |
| Watcher → API | `command.files.delete.progress` | Résultat par fichier (status deleted/failed, dirs_removed, error) |
| Watcher → API | `command.files.delete.completed` | Résumé final (total, deleted, failed) |

**Table `deletion_requests`** : Stocke chaque commande de suppression envoyée au watcher pour :
- Corréler les réponses du watcher avec l'opération initiale (via request_id)
- Renvoyer les commandes en attente à la reconnexion du watcher
- Tracker le status de la suppression physique indépendamment de la suppression planifiée
