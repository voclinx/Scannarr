...
use App\Service\RadarrConfigService;
...
{
    public function __construct(private readonly RadarrConfigService $radarrConfigService)
    {
    }
    ...
    {
        $data = $this->radarrConfigService->list();
    ...
    public function create(Request $request): JsonResponse
    {
        ...
        $result = $this->radarrConfigService->create($data);
    ...
    public function update(string $id, Request $request): JsonResponse
    {
        ...
        $result = $this->radarrConfigService->update($id, $data);
        if ($result === null) {
    ...
    {
        if (!$this->radarrConfigService->delete($id)) {
            return $this->json(['error' => ['code' => 404, 'message' => 'Radarr instance not found']], Response::HTTP_NOT_FOUND);
        }
    ...
    {
        $result = $this->radarrConfigService->testConnection($id);
        if ($result === null) {
            return $this->json(['error' => ['code' => 404, 'message' => 'Radarr instance not found']], Response::HTTP_NOT_FOUND);
        }
        if ($result['success']) {
    ...
    {
        $result = $this->radarrConfigService->rootFolders($id);
        if ($result === null) {
            return $this->json(['error' => ['code' => 404, 'message' => 'Radarr instance not found']], Response::HTTP_NOT_FOUND);
        }
        if ($result['result'] === 'error') {
            return $this->json(['error' => ['code' => 400, 'message' => sprintf('Failed to get root folders: %s', $result['error'])]], Response::HTTP_BAD_REQUEST);
        }
